...
int main() {
    ...
    loadKittiData(kitti_calibration, imu_measurements, gps_measurements,
                  ...
                  "/home/jerry/Desktop/GVIO/kittiData/KittiEquivBiasedImu.txt",
                  "/home/jerry/Desktop/GVIO/kittiData/KittiGps_converted.txt");
    ...
    auto noise_model_gps = noiseModel::Diagonal::Precisions((Vector6() << Vector3::Constant(0),
            Vector3::Constant(1.0/0.07)).finished());

    auto noise_model_gps_new = noiseModel::Diagonal::Precisions((Vector3() << Vector3::Constant(1.0/0.07)).finished());
    ...
    for (size_t i = first_gps_pose; i < gps_measurements.size() - 1; i++) {
        ...
        if (i == first_gps_pose) {
            ...
            auto gps_pose_new = Point3(gps_measurements[i].position);

            if ((i % gps_skip) == 0) {
                //new_factors.emplace_shared<PriorFactor<Pose3>>(current_pose_key, gps_pose, noise_model_gps);
                new_factors.emplace_shared<GpsFactor<Pose3>>(current_pose_key, gps_pose_new, noise_model_gps_new);
